...
**I&E staff email(s) that receive “someone requested admin access” notifications**

- When a user who is **not** in the allowed-admins list signs in at **Admin sign in**, the app records their request and sends the notification to every address listed here (comma-separated).
...
- Use one email or comma-separated list (e.g. `Zoe.Unsell@colorado.edu,Dahni.Austin@colorado.edu,...`) so all receive the request. If unset, requests are still recorded but no email is sent.

So: **ADMIN_REQUEST_NOTIFY_EMAIL** = one or more emails (comma-separated) that receive admin access requests.
...
| **ADMIN_REQUEST_NOTIFY_EMAIL** | I&E staff email(s), comma-separated; all receive admin access requests (optional but recommended) |
